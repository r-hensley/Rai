...
class Reports(commands.Cog):
    ...
    async def report(self, ctx, *, user=None):
        ...
        if guild_id not in self.bot.db['report']:
            await utils.safe_send(ctx, "This server has not run the setup for the report function yet.  Please type "
                                  "`;report setup`.")
            return
        ...
        if str(reaction.emoji) == '4⃣':  # Cancel
            await utils.safe_send(ctx.author, 'Understood.  Have a nice day!')
            return
    ...
    async def report_setup(self, ctx):
        ...
        if guild_id in self.bot.db['report']:
            ...
            await utils.safe_send(ctx, f"Successfully set the report room channel to {ctx.channel.mention}.")
        else:
            self.bot.db['report'][guild_id] = {'channel': ctx.channel.id,
                                               'current_user': None,
                                               'waiting_list': [],
    ...
    async def check_waiting_list(self, ctx):
        ...
        await utils.safe_send(ctx, message)
    ...
    async def clear_waiting_list(self, ctx):
        ...
        if config['waiting_list']:
            config['waiting_list'] = []
            await utils.safe_send(ctx, 'Waiting list cleared')
        else:
            await utils.safe_send(ctx, 'There was no one on the waiting list.')
    ...
    async def report_disable(self, ctx):
        ...
        if str(ctx.guild.id) in self.bot.db['report']:
            ...
            try:
                await utils.safe_send(ctx, "I've disabled the report module in your guild.")
            except discord.Forbidden:
                pass
        else:
            try:
                await utils.safe_send(ctx, "You don't have the report module enabled.")
            except discord.Forbidden:
                pass
    ...
    async def report_reset(self, ctx):
        ...
        config['waiting_list'] = []
        await utils.safe_send(ctx,
                              "The report module has been reset on this server.  Check the permission overrides on the "
                              "report channel to make sure there are no users left there.")
    ...
    async def report_anonymous_ping(self, ctx):
        ...
        if config['anonymous_ping']:
            await utils.safe_send(ctx, "Enabled pinging for anonymous reports.  I'll add a `@here` ping to the next one.")
        else:
            await utils.safe_send(ctx, "Disabled pinging for anonymous reports.")
    ...
    async def report_options(ctx, report_text):
        ...
        try:
            ...
            await utils.safe_send(ctx, "I'm unable to complete your request, as the user does not have PMs "
                                  "from server members enabled.")
    ...
    async def anonymous_report(ctx, report_text):
        ...
        await utils.safe_send(ctx.author, report_text[1])  # Instructions for the anonymous report
    ...
    async def report_room(ctx, config, user, report_text, from_mod=False):
        ...
        if config['current_user']:  # if someone is in the room already
            ...
            msg = "Sorry but someone else is using the room right now.  I'll message you when it's ope" \
                "n in the order that I received requests.  You are position " \
                "{config['waiting_list'].index(user.id)+1} on the list"
            ...
            try:
                ...
                await report_room.send("Note to the mods: I tried to send you a notification about the report room, "
                                       "but you haven't set a mod channel yet.  Please type `;set_mod_channel` in "
                                       "your mod channel.")
            return
        ...
        try:
            ...
            await user.send("The bot is not properly setup on this server. Please communicate with the mods "
                            "in another way.")
            await report_room.send("WARNING: A user tried to enter the room, but I lacked the permission to edit "
                                   "this channel to let them in. Please allow me to edit channel permissions.")
            config['current_user'] = None
            return
