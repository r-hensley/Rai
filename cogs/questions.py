...
class Questions(commands.Cog):
    ...
    async def grammar(self, ctx, *, search_term=None):
        ...
        data = await utils.aiohttp_get_text(url, ctx)
    ...
    async def neko(self, ctx, *search):
        ...
        # html = await utils.aiohttp_get_text('https://itazuraneko.neocities.org/grammar/masterreference.html', ctx)
        html = await utils.aiohttp_get_text('https://djtguide.github.io/grammar/masterreference.html', ctx)
    ...
    async def bunpro(self, ctx, search):
        ...
        # html = await utils.aiohttp_get_text('https://itazuraneko.neocities.org/grammar/masterreference.html', ctx)
        html = await utils.aiohttp_get_text('https://bunpro.jp/grammar_points', ctx)
    ...
    async def get_massif_results(self, ctx, search_term):
        ...
        text = await utils.aiohttp_get_text(url, ctx)
        if not text:
            await utils.safe_send(ctx, embed=utils.red_embed("No results found."))
            return
