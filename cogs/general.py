...
class HardcoreConfirmView(discord.ui.View):
    """Confirmation view for enabling percentage-gated hardcore mode."""

    def __init__(self, author: discord.Member, threshold: int, target_lang: str):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            "Only the person who invoked this command can interact with this.",
            ephemeral=True
        )
        return False

    @discord.ui.button(label="Enable Hardcore", style=discord.ButtonStyle.danger)
...
class General(commands.Cog):
                ...
                for command in self.bot.cogs[cog].get_commands():
                    if await check_command(command):
                        if isinstance(command, commands.Group):
                            ...
                            if to_append[1]:
                                cmd_dict[cog].append(
                                    f"`{to_append[0]}` (`{'`, `'.join(sorted(to_append[1]))}`)")
                            else:
    ...
    async def topic(self, ctx):
        ...
        config = {}
        if ctx.guild:
            config: dict = self.bot.db['topics'].setdefault(
                str(ctx.guild.id), {})
    ...
    async def hardcore(self, ctx: commands.Context, threshold: int = 0):
        """Adds/removes the hardcore role from you.

        On the Spanish server, you can optionally specify a percentage threshold (e.g. `;hc 50`)
        to lock yourself in until you reach that percentage of messages in your target language.
        Once locked in, you cannot remove hardcore until you meet the threshold."""
        if ctx.guild.id == SP_SERVER_ID:
            await self.hardcore_spanish_serv(ctx, threshold)
        else:
    ...
    async def hardcore_spanish_serv(self, ctx: commands.Context, new_threshold: int = 0):
        ...
        # check if user currently has hardcore
        hc_config: dict = self.bot.db['hardcore'].setdefault(str(SP_SERVER_ID), {})
        ...
        for r in all_roles:
            if r in ctx.author.roles:
                currently_has_hardcore = True
                current_role = r
                break

        # current user threshold
        current_threshold = current_user_config.get('threshold', 0)

        # user is trying to set a threshold, check if it's a valid value
        if new_threshold:
            # Validate threshold
            ...
            if new_threshold <= current_threshold:
                await utils.safe_send(
                    ctx,
                    f"You're already locked at **{current_threshold}%**. "
                    f"You can only raise your threshold, not lower it."
                )
                return

            # Check if user already meets the threshold
            current_pct = hf.get_language_percentage(ctx.author.id, ctx.guild, target_lang) or 0
            already_past = (current_pct) >= new_threshold

            if already_past:
                message = (
                    f"You already have **{current_pct}%** in **{lang_name}** "
                    f"(requested threshold: {new_threshold}%), so hardcore would have no lock-in effect. "
                    f"Enable anyway?"
                )
            else:
                message = (
                    f"You are about to enable hardcore mode locked to **{new_threshold}%** "
                ...
                if new_threshold == 100:
                    message += ("\n-# Note: A 100% threshold is effectively impossible, "
                                "so this will be a permanent lock.")

            view = HardcoreConfirmView(ctx.author, new_threshold, target_lang)
        ...
        # Threshold is a valid integer value
        if new_threshold < 50:
            role = hardcore_role  # give this role also when no threshold is set
        elif 50 <= new_threshold < 90:
            role = super_hardcore_role
        else:
            role = ultra_hardcore_role

        # User has hardcore mode with no threshold set: simply remove the role
        if currently_has_hardcore and not current_threshold and not new_threshold:
            users_config.pop(str(ctx.author.id), None)
            for r in all_roles:
                if r in ctx.author.roles:
        ...
        # User with threshold is removing their own hardcore (check if they reached their goal)
        elif currently_has_hardcore and current_threshold and not new_threshold:
            target_lang = current_user_config['target_lang']
            lang_name = {'es': 'Spanish', 'en': 'English'}.get(target_lang, target_lang)
            current_pct = hf.get_language_percentage(ctx.author.id, ctx.guild, target_lang) or 0
            ...
            if current_pct < current_threshold:
                ...
                # Met the threshold — allow removal and clean up
                await ctx.author.remove_roles(current_role)
                await utils.safe_send(
                    ctx,
                    f"Congrats on reaching your goal of **{current_threshold}%**! "
        ...
        # User is trying to raise their current threshold / set a new threshold
        elif new_threshold:
            users_config[str(ctx.author.id)] = {
            ...
            await utils.safe_send(
                ctx,
                f"Hardcore enabled! You must reach **{new_threshold}%** in "
                f"**{lang_name}** before you can remove it. During this time, you can only speak in {lang_name}."
            )
            return

        # User is just adding simple hardcore (no threshold): add the role simply
        else:
            await ctx.author.add_roles(role)
            await utils.safe_send(
                ctx,
                f"I've added hardcore to you. You can now only speak in {lang_name}. Type `;hc` again to "
                f"remove this. You can also set a percentage goal for yourself by typing something like `;hc 30`."
            )
            return

    async def hardcore_other_serv(self, ctx: commands.Context):
        role = ctx.guild.get_role(self.bot.db['hardcore'][str(ctx.guild.id)]['role'])

        if role in ctx.author.roles:
            # User wants to REMOVE hardcore
            ...
            # User wants to add hardcore role
            await ctx.author.add_roles(role)
            await utils.safe_send(
                ctx,
                "I've added hardcore to you. You can only speak in the language you're learning."
            )
    ...
    async def serverinfo(self, ctx):
        ...
        em.add_field(
            name="Channels", value=f"{len(guild.text_channels)} text / {len(guild.voice_channels)} voice")
        em.add_field(name="Verification Level", value=guild.verification_level)
        em.add_field(name="Guild created on (UTC)",
        ...
        if len(guild.members) < 60000:
            ...
            top_six_roles = '\n'.join(
                f"{role}: {count}" for role, count in role_count.most_common(6))
            em.add_field(
                name=f"Top 6 roles (out of {len(guild.roles)})", value=top_six_roles)
        else:
    ...
    async def blacklist_remove(self, ctx, entry_message_id):
        ...
        emb.set_field_at(0, name="Entry removed by",
        ...
        for user in list_of_ids:
            ...
            async def post_vote_notification(target_user, reason):
                ...
                if not target_user:
                    target_user = ''
                emb = discord.Embed(
                    title=f"{user} {target_user} (1 vote)", color=discord.Color(int('ffff00', 16)))
            ...
            try:  # the guild ID that the person trying to add a vote belongs to
                user_residency = config['residency'][str(
                    ctx.author.id)]  # a guild id
            except KeyError:
            ...
            if user not in config['votes']:  # 0 votes
                config['votes'][user] = {
                    'votes': [user_residency], 'message': 0}
            ...
            if user in config['votes']:  # 1, 2, or 3 votes
                ...
                emb.title = re.sub(
                    '(.) vote', f'{int(num_of_votes) + 1} vote', emb.title)
    ...
    async def blacklist_bansub(self, ctx):
        ...
        subbed_roles: list = self.bot.db['bansub']['user_to_role'].setdefault(
            str(ctx.author.id), [])
        user_role_ids = [role.id for role in ctx.author.roles if str(
            role.color) == "#3498db"]  # only want blue roles
        selection_dictionary = {}  # for later when the user selects a role to toggle
        # links a guild ID to the corresponding role
        guild_id_to_role: dict = self.bot.db['bansub']['guild_to_role']
        # reverses the dictionary
        role_to_guild_id = {guild_id_to_role[a]: a for a in guild_id_to_role}
            ...
            if role_selection not in role_to_guild_id:
                ...
                if re.search(r'^\d{17,22}$', resp):  # user specifies a guild ID
                    ...
                    guild = discord.utils.find(
                        lambda g: g.name == resp, self.bot.guilds)
                    if not guild:
    ...
    async def lsar(self, ctx, page_num=1):
        ...
        if len(roles_list) == 1:
            role_list_str = role_list_str.replace(
                'roles', 'role').replace('are', 'is')
        current_group = ''
        try:
        ...
        emb = discord.Embed(description=role_list_str,
    ...
    def iam_find_role(ctx, r_name):
        ...
        found_role: Optional[discord.Role] = discord.utils.find(
            lambda r: r.name.casefold() == r_name, ctx.guild.roles)
        if not found_role:
            if 3 <= len(r_name):
        ...
        while time_left:
            ...
            try:
                ...
                if str(reaction_added) == '❌':
                    print("oops", reaction_added.message.content,
                          reaction_added.message.author)
                    aborted = True
                    time_left = 0
    ...
    @app_commands.choices(format_option=[
        ...
        app_commands.Choice(
            name='Short Date+Time (June 30 2021, 9:41 PM)', value='f'),
        app_commands.Choice(
            name='Long Date+Time (Wednesday, June 30, 2021, 9:41 PM)', value='F'),
        app_commands.Choice(
            name='Relative (2 months ago, in an hour, in five years)', value='R'),
    ])
