...
class Events(commands.Cog):
    ...
    async def on_message(self, msg: discord.Message):
        ...
        async def jho3_easy():
            if msg.channel.id != 1224420644746301643:
                return

            # add "0" to a count for each letter in a message in first half of the alphabet
            # add "1" to a count for each letter in a message in the last half of the alphabet
            count = 0
            for letter in msg.content.casefold():
                if letter in "abcdefghijklm":
                    count -= 1
                elif letter in "nopqrstuvwxyz":
                    count += 1

                # include japanese hiragana too, first half of hiragana is "0", second half is "1"
                elif letter in "あいうえおかきくけこさしすせそたちつてとがぎぐげござじずぜぞだぢづでど":
                    count -= 1
                elif letter in "なにぬねのはひふへほまみむめもやゆよらりるれろわをんばびぶべぼぱぴぷぺぽ":
                    count += 1
                # katakana too
                elif letter in "アイウエオカキクケコサシスセソタチツテトガギグゲゴザジズゼゾダヂヅデド":
                    count -= 1
                elif letter in "ナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンバビブベボパピプペポ":
                    count += 1

                # numbers, 0-4 is +0, 5-9 is +1
                elif letter in "01234":
                    count -= 1
                elif letter in "56789":
                    count += 1
        ...
        async def jho3_medium():
            # messages must have the following three requirements
            # 1) must contain both english and japanese characters
            # 2) the number of english characters is the square of the number of japanese characters
            if msg.channel.id != 1224432057078386850:
                return
            count_english = 0
            count_japanese = 0

            # for checks of engilsh/japanese, use hf.is_english(char) and hf.is_cjk(char)
            for letter in msg.content:
                if hf.is_english(letter):
                    count_english += 1
                elif hf.is_cjk(letter):
                    count_japanese += 1

            if count_english > 0 and count_japanese > 0:
                if count_english == count_japanese ** 2:
                    if count_english % 2 == 0:
                        if count_japanese % 2 == 0:
                            return
        ...
        async def jho3_medium_hard():
            # in this challenge, users must:
            # 1) have at least one repeating character somewhere in their message
            # 2) must include somewhere in their message the highest repeated character from the previous message
            # 3) must contain any one of the letters "JHOjhoじはひふへほお3３三" in their message
            if msg.channel.id != 1224442809193332869:
                return

            if not hasattr(self.bot, 'highest_repeated'):
                self.bot.highest_repeated = 'a'

            if not msg.content:
                return

            if msg.content.startswith(";echo") and msg.author.id == self.bot.owner_id:
                return

            delete = False
            # get the highest repeated character from the current message
            ...
            potential_highest_repeated = max(count_dict, key=count_dict.get)
            if count_dict[potential_highest_repeated] != count_dict.get(self.bot.highest_repeated, 0):
                highest_repeated = potential_highest_repeated
            else:
                highest_repeated = self.bot.highest_repeated

            # highest_repeated must be in the alphabet or cjk
            if not (hf.is_english(highest_repeated) or highest_repeated == 'w') and not hf.is_cjk(highest_repeated):
                delete = f"**{msg.content}**, Repeated letter ({highest_repeated}) not in alphabet or CJK"

            if highest_repeated != self.bot.highest_repeated:
                if count_dict[highest_repeated] == 1:
                    delete = f"**{msg.content}**, Repeated letter ({highest_repeated}) only appears once"

            # check if the highest repeated character matches what is in bot.highest_repeated
            if self.bot.highest_repeated not in content:
                delete = f"**{msg.content}**, Previous highest repeated letter ({self.bot.highest_repeated}) not in message"

            # check for required characters
            required_chars = "jhoじはひふへほお3３三"
            found_required_char = False
            for char in required_chars:
                if char in content:
                    found_required_char = True
                    break
            if not found_required_char:
                delete = f"**{msg.content}**, Required character not found in message"

            # await hf.safe_reply(msg, f"{count_dict}, {highest_repeated}, "
            #                          f"{found_required_char}, {delete}, {self.bot.highest_repeated}")
            if delete:
                await hf.send_to_test_channel(f"({msg.author.name}) [{self.bot.highest_repeated}], {delete}")
                try:
                    await msg.delete()
                except (discord.Forbidden, discord.HTTPException):
                    pass

            # store the highest repeated character in bot.highest_repeated
            if not delete:
                await hf.send_to_test_channel(f"[PASS], ({msg.author.name}) **{msg.content}**, [{self.bot.highest_repeated} --> {highest_repeated}]")
                self.bot.highest_repeated = highest_repeated

        await jho3_medium_hard()

        async def jho3_hard():
            if msg.channel.id != 1221873617189736458:
                return
            hash_input = msg.content
            hash_output = hashlib.sha256(hash_input.encode()).hexdigest()
            if hash_output[-1] not in '0123':
                try:
                    await msg.delete()
                except (discord.Forbidden, discord.HTTPException):
                    pass
