...
Estonia"""
        ...
        print(y)
        roles = []
        g = self.bot.get_guild(243838819743432704)
        for rolename in y:
            role: discord.Role = discord.utils.get(g.roles, name=rolename)
            if not role:
                print(f"Could not find {rolename}")
            else:
                roles.append(role)
        ...
        for role in roles:
            name = role.name
            countrycode = country_to_abbrev[name].casefold()
            emoji = f"{reg[countrycode[0]]}{reg[countrycode[1]]}"
            # print(emoji)
        ...
        for member in g.members:
            ...
            for role in roles:
                if role in member.roles:
                    emoji_to_add = roletoemoji[role]
                    if emoji_to_add not in member.display_name:
                        pass
                    try:
                        await member.edit(nick=member.display_name + emoji_to_add)
                        # print(f"{str(member)}: added {role.name}'s {emoji_to_add}")
                    except discord.Forbidden as e:
                        print(f"{str(member)}: {e}")
                    except discord.HTTPException as e:
                        print(f"{str(member)}: {e}")
            if count % 5000 == 0:
                print(f"{count} / {len(g.members)}")
